...
Write Agent has only MCP filesystem tools, single responsibility: **write code to a file**. After writing, it calls `list_directory` to confirm the file exists — this gives the LLM a clear completion signal and prevents it from rewriting the file repeatedly.
