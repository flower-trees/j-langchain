...
> 相关资源
> - 完整代码：[Article27HumanInTheLoop.java](../../../src/test/java/org/salt/jlangchain/demo/article/Article27HumanInTheLoop.java)（`testUserConfirmYes` / `testUserConfirmNo` / `doConfirmFlow` 方法）
> - 前置阅读：[Agent 运行时停止类型：MAX_STEPS、TIMEOUT、CONSECUTIVE_TOOL_FAILURES 与 AgentPauseException](26-agent-stop-types.md)
